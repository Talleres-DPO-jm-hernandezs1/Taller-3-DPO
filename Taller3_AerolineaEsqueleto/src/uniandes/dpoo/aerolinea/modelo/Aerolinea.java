...
public class Aerolinea
{
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	if (getVuelo(codigoRuta, fecha) != null) {
    		throw new Exception("Ya existe un vuelo programado para esta fecha y ruta");
    	}
    	
    	for (Vuelo vuelo : vuelos) {
    		if(vuelo.getFecha().equals(fecha) && vuelo.getAvion().equals(nombreAvion)) {
    			throw new Exception("El vuelo con este avión ya esta programado para esta fecha");
    		}
    	}
    	...
    	Avion avion = null;
    	for (Avion a : aviones) {
    		if (a.getNombre().equals(avion));
    		avion = a;
    		break;
    	}
    	
    	Vuelo vueloprogramado = new Vuelo(ruta, fecha, avion);
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
    	...
    	int precioTiquete = calculadora.CalcularTarifa(vuelo, cliente);
